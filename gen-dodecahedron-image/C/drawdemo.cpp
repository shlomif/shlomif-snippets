/****************************************************************************
** $Id: drawdemo.cpp,v 2.7.2.1 1998/11/16 10:26:55 hanord Exp $
**
...
void drawColorWheel( QPainter *p )
{
    ...
    if ( p->device()->devType() == PDT_PRINTER )
	f.setPointSize( (f.pointSize()*500)/p->viewport().height() );
    p->setFont( f );
...
void drawLines( QPainter *p , char * filename)
{
    p->setPen(black);
    ...
    {
        sscanf(string, "%i %i %i %i %i %i %i", &x1, &y1, &x2, &y2, &r, &g, &b);
...
void drawDodeca(QPainter *p)
{
    drawLines(p,"dodeca.txt");
}

void drawCorner(QPainter *p)
{
    drawLines(p,"corner.txt");
}
...
DrawThing ourDrawFunctions[] = {
    ...
    { drawDodeca,       "Draw Dodecahedron" },
    { drawCorner,       "Draw Corner" },
    { 0,		0 } };
